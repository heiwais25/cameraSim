...
// Counting event written in the log to make each file's maximum size to 200MB
...
// Write Photon hit on the dom information by hex
// each event will take 32 bytes
void writeHitInfo(const name & n, const hit & h){
  ...
  outFile.write((char *)&dom,sizeof(float));
  outFile.write((char *)&h.t,sizeof(float));
  outFile.write((char *)&h.z,sizeof(float));
  outFile.write((char *)&h.pth,sizeof(float));
  outFile.write((char *)&h.pph,sizeof(float));
  outFile.write((char *)&h.dth,sizeof(float));
  outFile.write((char *)&h.dph,sizeof(float));
  eventCount++;
  if(eventCount == 1024 * 1024 * 8){
    closeFile();
    eventCount = 0;
    openFile();
  }
